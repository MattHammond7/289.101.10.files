...
document.getElementById('calculate').addEventListener('click', function() {
  ...
  if (width<=0) {
    alert("Negative width value")
    document.getElementById('triangle').setAttribute('width', '0');
  }
  document.getElementById('triangle').setAttribute('height', height);
  if (height<=0) {
    alert("Negative height value")
    document.getElementById('triangle').setAttribute('height', '0');
  }
  document.getElementById('hypotenuse').value = Math.sqrt(height*height+width*width);
